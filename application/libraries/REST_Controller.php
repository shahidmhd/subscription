...
abstract class REST_Controller extends CI_Controller {
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        {
            // added by SANISH PS  to solve OPTIONS verbs send by client like BACKBONE.JS
            // if method is OPTIONS reply with 204(no content) so that client will resend the request with supported verbs(get, post...)
            // @auther : Sanish P S
            if($this->request->method == 'options') {
                $this->response([config_item('rest_status_field_name') => FALSE, config_item('rest_message_field_name') => 'OPTIONS methods. Try next request.'], 204);     
            }else{
    ...
    {              
        ...
        */
        //if ( "OPTIONS" === $_SERVER['REQUEST_METHOD'] ) { } 
             ...
             //if($_SERVER['HTTP_ORIGIN'] != null) $http_orgin = $_SERVER['HTTP_ORIGIN'];  
             header('Access-Control-Allow-Origin: '.$http_orgin); 
